...
if handles.setclus == 0 && handles.undo==0 && handles.merge(1)==0 && handles.force==0  
...
%Get for merge
 if handles.merge(1) == 1 
     merge_class = [find(classes==handles.merge(2)) find(classes==handles.merge(3))];
 end
...
% Merge operations (Original)
% if handles.merge(1) == 1 && ~isempty(nfix_class)
...
%%%%New Merge method with only 2 clusters
 if handles.merge(1) == 1 
    imerge = 1;% index for the original temperature that will represent all the fixed classes
    bigger_fix = 0;
    for i = 1:2;
        aux = nnz(clustering_results(:,2) == handles.merge(i+1));
        if aux > bigger_fix
            imerge = i;
            bigger_fix = aux;
        end
    end
    imerge = find(clustering_results(:,2) ==handles.merge(imerge+1),1);
    mtemp = clustering_results(imerge,3); % temperature that represents all the fixed classes
    classes(merge_class) = handles.merge(2); % labels all the fixed classes with the new number
...
if handles.merge(1) == 1 && ~isempty(merge_class)
    clustering_results(merge_class,3) = mtemp;
    clustering_results(merge_class,4) = clustering_results(imerge,4);
end 
...
if (~isempty(fix_class2)) && handles.merge(1)==0 && handles.undo==0 && handles.force==0
...
if isempty(fix_class2) && handles.undo==0 && handles.merge(1)==0 && handles.force==0
    if isfield(handles,'new_manual')
...
for i = 0:nclusters
    if ~ (isempty(class0) && i==0)
        ...
        if i < 4
            ...
            set(handles.wave_clus_figure,'userdata',USER_DATA)
            if i < 9
                opened_figs{1} = wave_clus_aux('Visible', 'off','WindowStyle','docked');
            elseif i < 14
                opened_figs{2} = wave_clus_aux1('Visible', 'off','WindowStyle','docked');
            elseif i < 19
                opened_figs{3} = wave_clus_aux2('Visible', 'off','WindowStyle','docked');
            elseif i < 24
                opened_figs{4} = wave_clus_aux3('Visible', 'off','WindowStyle','docked');
            elseif i < 29
                opened_figs{5} = wave_clus_aux4('Visible', 'off','WindowStyle','docked');
            elseif i < 34
                opened_figs{6} = wave_clus_aux5('Visible', 'off','WindowStyle','docked');
            %-------------------------------------------------------------------------
            end
        end
    end
...
%Recheck fixed clusters <--- always placed at end 

for fixed = 0:length(nfix_class)-1
    toFix = nclusters-fixed;
    fix_class = find(classes==toFix);
    USER_DATA{19+toFix} = fix_class;
    if toFix<4
        set(eval(['handles.fix' num2str(toFix) '_button']),'value',1);
    else
        if toFix<9
            aux ='wave_clus_aux';
            auxfigs = guidata(findobj(get(0,'children'), 'tag',aux));
        else
            aux = ['wave_clus_aux' num2str(ceil((toFix-3)/5)-1)];
            auxfigs = guidata(findobj(get(0,'children'), 'tag',aux));
        end
        
        set(eval(['auxfigs.fix' num2str(toFix) '_button']),'value',1)
        eval(['par.fix' num2str(toFix) '= 1;'])
    end
    
end
USER_DATA{1} = par;
%Ensure all clusters have the same tag
%Use mode (ie change temp, merge)
%Edit Tags
tags = USER_DATA{17};
tags(:,2) = tags(:,1);
for i = 0:nclusters
    tags(classes==i,1) = mode(tags(classes==i));
end
USER_DATA{17} = tags;
set(handles.wave_clus_figure,'userdata',USER_DATA);
...
clear classes clustering_results

...
{	
	...
	etu::AnalogOut(0, (float)(g_pGratings->getX(currentTrial->leftEyeFixating) + g_pSettings->getXOffset() + m_gridXDeg) / g_pSettings->getDegPerVolt());
	etu::AnalogOut(1, (float)(g_pGratings->getY(currentTrial->leftEyeFixating) + g_pSettings->getYOffset() + m_gridYDeg) / g_pSettings->getDegPerVolt());
...
void CGratingsStimulus::drawBlankScreen(bool displayDot, bool displayFixLeft) 
{
	...
	/* Get fixation dot size, positions, and offsets */
	/** 
	 * @TODO Converting degree quantities to pixels and then adding is 
	 * incorrect.  It introduces ~2 percent additional error to tangent 
	 * approximation. 
	 */
	g_pSettings->tanScreenUnit2Unit(vsgDEGREEUNIT,  g_pGratings->getDotSize(), vsgPIXELUNIT, &dotDiameter);
	g_pSettings->tanScreenUnit2Unit(vsgDEGREEUNIT,  g_pGratings->getX(displayFixLeft), vsgPIXELUNIT, &dotX);
	g_pSettings->tanScreenUnit2Unit(vsgDEGREEUNIT, -g_pGratings->getY(displayFixLeft), vsgPIXELUNIT, &dotY);
	...
	if (displayDot) {
		double xCoord = dotX + offsetX + m_gridXPix;
		double yCoord = dotY + offsetY + m_gridYPix;

		vsgSetPen1(1);
		vsgDrawOval(xCoord + screenOffsetX, yCoord + screenOffsetY, dotDiameter, dotDiameter);

		etu::AnalogOut(0, (float)(xCoord) / g_pSettings->getDegPerVolt());
		etu::AnalogOut(1, (float)(yCoord) / g_pSettings->getDegPerVolt());
	} 
	else {
									...
									double tmpAvgRY)
{
	...
	/* Get fixation dot size, positions, and offsets */
	/** 
	 * @TODO Converting individual quantities to pixels and then adding is 
	 * incorrect.  It introduces ~2 percent additional error to tangent 
	 * approximation. 
	 */
	g_pSettings->tanScreenUnit2Unit(vsgDEGREEUNIT,  g_pGratings->getDotSize(), vsgPIXELUNIT, &dotSize);
	g_pSettings->tanScreenUnit2Unit(vsgDEGREEUNIT,  g_pGratings->getXLeft(), vsgPIXELUNIT, &leftX);
	g_pSettings->tanScreenUnit2Unit(vsgDEGREEUNIT, -g_pGratings->getYLeft(), vsgPIXELUNIT, &leftY);
	g_pSettings->tanScreenUnit2Unit(vsgDEGREEUNIT,  g_pGratings->getXRight(), vsgPIXELUNIT, &rightX);
	g_pSettings->tanScreenUnit2Unit(vsgDEGREEUNIT, -g_pGratings->getYRight(), vsgPIXELUNIT, &rightY);
	g_pSettings->tanScreenUnit2Unit(vsgDEGREEUNIT,  g_pSettings->getXOffset(), vsgPIXELUNIT, &offsetX);
	g_pSettings->tanScreenUnit2Unit(vsgDEGREEUNIT, -g_pSettings->getYOffset(), vsgPIXELUNIT, &offsetY);
	g_pSettings->tanScreenUnit2Unit(vsgDEGREEUNIT,  g_pSettings->getScreenXOffset(), vsgPIXELUNIT, &screenOffsetX);
	g_pSettings->tanScreenUnit2Unit(vsgDEGREEUNIT, -g_pSettings->getScreenYOffset(), vsgPIXELUNIT, &screenOffsetY);
	...
	if (drawGabor) {
		...
		cutout2[pointsOnCircle*2 + 10] = cutout2[pointsOnCircle*2 + 1];
	...
	/* Fixation dot */
	vsgSetPen1(1);
	vsgDrawOval(((trial->leftEyeFixating) ? leftX : rightX) + totalOffsetX, 
		((trial->leftEyeFixating) ? leftY : rightY) + totalOffsetY, dotSize, dotSize);

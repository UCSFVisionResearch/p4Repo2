...
bool CFullfieldStimulus::Initialize() 
{
	...
	if (g_pFullfield->getNoise()) {
		...
		lutbuf[1].a = dotColor.a;
		lutbuf[1].b = dotColor.b;
		lutbuf[1].c = dotColor.c;
		...
		vsgDrawNoise(iScrWidthPix/2, iScrHeightPix/2, iScrWidthPix, iScrHeightPix,
			g_pFullfield->getBarWidth(), g_pFullfield->getBarWidth(), 0, 1, 3 );
		...
		g_stim = vsgObjCreate();
		vsgObjSetContrast(100);//g_pFullfield->getContrast());
		...
		vsgObjSetColourVector(&dotColor, &blackColor,vsgBIPOLAR);

            ...
            DispatchMessage(&msg); 
				...
				if (msg.message == WM_KEYDOWN) {
					...
					if (g_pEyeTrackingDlg->m_pTabCtrl.GetCurSel()==9) {
						/* RF mapping dialog */
						if (g_pRFMapping->useMouse() && g_pActiveStimulus  && \
							...
							if (zDelta != 0) {
								...
								Sleep(1);
								g_pRFMappingDlg->m_XPosition.SetWindowText(etu::FloatToString(g_pRFMapping->getXPosition()));
								g_pRFMappingDlg->m_YPosition.SetWindowText(etu::FloatToString(g_pRFMapping->getYPosition()));
								g_pRFMappingDlg->m_BarWidth.SetWindowText(etu::FloatToString(g_pRFMapping->getBarWidth()));
								g_pRFMappingDlg->m_BarHeight.SetWindowText(etu::FloatToString(g_pRFMapping->getBarHeight()));
								g_pRFMappingDlg->m_Orientation.SetWindowText(etu::FloatToString(g_pRFMapping->getOrientation()));
								g_pRFMappingDlg->m_CircleRadius.SetWindowText(etu::FloatToString(g_pRFMapping->getCircleRadius()));
							}
						}
					}
				}

...
: m_xPos(0), m_yPos(0), m_size(size), m_lutBase(lutBase), m_percent(percent), 
...
: m_xPos(xPos), m_yPos(yPos), m_width(width), m_height(height), m_size(size),
  m_lutBase(lutBase), m_percent(percent), m_lutBuffer(lutBuffer), 
  m_occlude(false), m_patch(0), m_refPageZone(0xFFFF), m_refPageNum(0xFFFF)
{
	/* Calls this->configureLut(), which is necessary. */
	this->setColors(bgColor, noiseColor);
}
...
{
	/* Set up LUT with 100 levels of BG and 100 levels of noise color.  Later, 
	 * to specify the range, start from 'm_lutBase + m_percent' and span 100
	 * levels.  Doing so will include 'm_percent' noise levels and
	 * '100 - m_percent' BG levels. */
	for (int ix = m_lutBase; ix < m_lutBase + 100; ix++) {
		m_lutBuffer[ix] = m_bgColor;
	}

	for (int ix = m_lutBase + 100; ix < m_lutBase + 200; ix++) {
		m_lutBuffer[ix] = m_noiseColor;
	}
}
...
{
	return (((m_lutBase + 100) <= level) && (level < (m_lutBase + 200)));
}
 ...
 * @param noiseColor Values to set for noise color; param not modified.
 */
void NoiseBgController::setColors(const VSGTRIVAL& bgColor, 
								  const VSGTRIVAL& noiseColor)
{
	m_bgColor = bgColor;
	m_noiseColor = noiseColor;
	this->configureLut();
}
 ...
 * @param[out] noiseColor Receives noise color values.
 */
void NoiseBgController::getColors(VSGTRIVAL& bgColor, VSGTRIVAL& noiseColor)
{
	this->getBgColor(bgColor);
	this->getNoiseColor(noiseColor);
}

/**
 * @brief Set new BG color.
 *
 * Causes class to reconfigure the LUT.
 *
 * @param color Values to set; param not modified.
 */
void NoiseBgController::setBgColor(const VSGTRIVAL& color)
{
	this->setColors(color, m_noiseColor);
}

/**
 * @brief Read BG color into variable.
 * @param[out] color Receives BG color values.
 */
void NoiseBgController::getBgColor(VSGTRIVAL& color) { color = m_bgColor; }

/**
 * @brief Set new noise color.
 *
 * Causes class to reconfigure the LUT.
 *
 * @param color Values to set; param not modified.
 */
void NoiseBgController::setNoiseColor(const VSGTRIVAL& color)
{
	this->setColors(m_bgColor, color);
}

/**
 * @brief Read noise color into variable.
 * @param[out] color Receives noise color values.
 */
void NoiseBgController::getNoiseColor(VSGTRIVAL& color) { color = m_noiseColor; }
...
void NoiseBgController::setPercent(int percent) { m_percent = percent; }
int NoiseBgController::getPercent() { return m_percent; }

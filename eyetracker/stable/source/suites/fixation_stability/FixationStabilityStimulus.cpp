...
void CFixationStabilityStimulus::InitNoiseHandling()
{
	...
	int noisePct = g_pFixationStabilityModel->noiseParameters().noiseCoverage();
	int commonPct = ((int)(noisePct * 
		(g_pFixationStabilityModel->noiseParameters().noiseCommon() / 100.0)));
	int monoc = (g_pFixationStabilityModel->noiseParameters().noiseCoverage() - commonPct);

	CleanUpNoiseHandling();

	m_noiseBgController = new VsgExt::NoiseBgController(size, m_bgColor, 
		m_noiseColor, LEVEL_BG_CONTR, commonPct, monoc, monoc, m_lutbuf);
	...
	/* Used for drawing the same BG throughout all trials. */
...
bool CFixationStabilityStimulus::DisplayStimulus() 
{
	...
	etu::AnalogOut(A_OUT_TARG_COLOR, ((currentTrial->eye() == fstab::LEFT_EYE) ? 
		(float)VOLTS_COLOR_LEFT : (float)VOLTS_COLOR_RIGHT));

	/* Prep text mark before stimulus starts to prevent time lags later. */
	{
		char* color = ((currentTrial->eye() == fstab::LEFT_EYE) ? 
			"Blue" : "Red");
		bool occ = currentTrial->occludeLeft() || currentTrial->occludeRight();
		char* occluded = (occ ? "occluded" : "");
		sprintf(buffer, "%s (%.2f, %.2f) %s|0x%x/\0", color, centerXDeg, 
			centerYDeg, occluded, currentTrial->getCode());
		stringSize = strlen(buffer);
	}

	SetShutters(currentTrial->occludeLeft(), currentTrial->occludeRight());

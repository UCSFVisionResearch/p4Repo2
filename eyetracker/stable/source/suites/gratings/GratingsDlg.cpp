...
void CGratingsDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_FixLocL, m_FixLocL);
	DDX_Control(pDX, IDC_FixLocR, m_FixLocR);
...
{
	m_FixLocL.SetWindowText((LPCSTR)g_pGratings->getLeftFixationString());
	m_FixLocR.SetWindowText((LPCSTR)g_pGratings->getRightFixationString());
...
{
	char buffer[FIXSTR_BUFLEN];
	int affected = 0;

	m_FixLocL.GetWindowText(buffer, FIXSTR_BUFLEN - 1);
	buffer[FIXSTR_BUFLEN - 1] = '\0';
	g_pGratings->setLeftFixations(buffer);
	
	m_FixLocR.GetWindowText(buffer, FIXSTR_BUFLEN - 1);
	buffer[FIXSTR_BUFLEN - 1] = '\0';
	g_pGratings->setRightFixations(buffer);

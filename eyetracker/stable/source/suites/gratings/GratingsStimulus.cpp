...
COMMUNICATIONS:
	...
	VSG Out 1:	behavioral contingency (vsg pin 2; 1401 pin 18; 1401 dig in 6)
				...
				has been moved.
	VSG Out 2:	which eye must fixate (VSG pin 3; 1401 pin 6; 1401 dig in 5)
				0: left eye
				1: right eye.
...
{	
	...
	CGratingsTrial* currentTrial;

	/* Get next trial and fixation point. Make the trial list circular if we've 
	 * indexed out. Use SmartLock because the stimulus executes in a different
	 * thread. */
	{
		SmartLock gratingsLock(*g_pGratings);

		if ((m_trialIndex < 0) || (m_trialIndex >= g_pGratings->numTrials)) m_trialIndex = 0;
		currentTrial = g_pGratings->trials[m_trialIndex]->clone();
		m_phase = currentTrial->phase;
		m_angle = currentTrial->orientation;
	}
	
	m_fixationPoint = fixPoint(currentTrial->leftEyeFixating);
	...
	if (m_firstInit) {
		...
		/** 
		 * @DEPRECATED
		 * The gratings paradigm was changed to use two lists of fixation 
		 * points, one for each eye, with some points duplicated. This is 
		 * incompatible with automated RF mapping.
		 * JVB 10/8/2015
		 */
	...
	vsgIOWriteDigitalOut((currentTrial->leftEyeFixating ? 0x0000 : 0xFFFF), vsgDIG2);
	etu::AnalogOut(0, (float)(m_fixationPoint->xStimDeg()) / g_pSettings->getDegPerVolt());
	etu::AnalogOut(1, (float)(m_fixationPoint->yStimDeg()) / g_pSettings->getDegPerVolt());
	if (g_pGratings->getBehavior() && (g_pGratings->getInitiationDelay() > 0)) 
	{
		...
		{
			if (vsgIOReadDigitalIn() & vsgDIG0) m_frameTimer.CheckTimerWithReset();
...
void CGratingsStimulus::drawBlankScreen(bool displayDot, bool displayFixLeft) 
{
	...
	if (displayDot) {
		...
		vsgSetPen1(1);
		vsgDrawOval(m_fixationPoint->xScreenPix(), m_fixationPoint->yScreenPix(), dotPix, dotPix);
									...
									double tmpAvgRY)
{
	...
	vsgSetPen1(1);
	vsgDrawOval(m_fixationPoint->xScreenPix(), m_fixationPoint->yScreenPix(), dotSize, dotSize);
...
const CSmartCoord* CGratingsStimulus::fixPoint(bool left)
{
	SmartLock gratingsLock(*g_pGratings);
	return (left ? g_pGratings->getRandomLeftFixation() : 
		g_pGratings->getRandomRightFixation());
}

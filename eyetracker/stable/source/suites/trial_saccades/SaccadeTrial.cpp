		...
		{	
			for (int eye = CSaccadeTrial::EYE_RIGHT; eye <= CSaccadeTrial::EYE_LEFT; ++eye)
			{
				for (int code = CSaccadeTrial::CODE_C_RED_P_RED; code <= CSaccadeTrial::CODE_C_BLUE_P_BLUE; ++code)
				{
					...
					numRepeats = (isOverMidline(pSaccadeTrial, m_eyePositions->GetDeviation()) 
						? numRepeatsCrossover : numRepeatsStandard);

					/* The loop will generate between 0 and numRepeatsStandard 

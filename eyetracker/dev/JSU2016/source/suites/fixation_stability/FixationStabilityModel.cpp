	...
	m_pNoiseParameters(NULL)
{
	...
	etu::CColorSpec left(0, 0, 255);
	etu::CColorSpec right(255, 0, 0);
	etu::CColorSpec noise(50, 0, 50);
	...
	m_pNoiseParameters = new etu::CNoiseParameters(true, true, false, false, 2, 50, 100, noise);
...
void CFixationStabilityModel::LoadFile(bool bDefault) 
{
	...
	etu::CNoiseParameters* noiseParams = NULL;
	...
	noiseParams = etu::CNoiseParameters::getFrom(fp);
											 ...
											 int deviation, bool occlude)
{
	...
	for ( ; begin < end; ++begin) {
		...
		m_trials.push_back(fstab::CFixationStabilityTrial(eye, xDeg, 0, 
			occludeLeft, occludeRight, (*begin)));
	}
...
const etu::CNoiseParameters& CFixationStabilityModel::noiseParameters() const
...
void CFixationStabilityModel::noiseParameters(etu::CNoiseParameters* noise)
{

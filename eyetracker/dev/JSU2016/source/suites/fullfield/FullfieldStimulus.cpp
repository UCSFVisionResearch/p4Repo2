...
bool CFullfieldStimulus::Initialize() 
{
	...
	if (g_pFullfield->getNoise()) {
		...
		dCalculatedFreq = g_pFullfield->getCirFreq() * 2 * 3.14159265358979323846 / g_dFramerate;// * 1.28; //M_PI
		g_pSettings->tanScreenUnit2Unit(vsgDEGREEUNIT, g_pFullfield->getCirAmp(), vsgPIXELUNIT, &dAmplitudePix);
...
bool CFullfieldStimulus::DisplayStimulus() 
{
	...
	if (g_pFullfield->getNoise()) {
		...
		while (FullFieldTimer->CheckTimer() < g_pFullfield->getDurationMSec()) {
			if(g_pFullfield->getDriftDirection()!=CFullfield::DIR_CIR){
				...
				y = SineWave(iFrameNum +floor(2*3.1415/(4*dCalculatedFreq)), dCalculatedFreq, dAmplitudePix)+dAmplitudePix;
				g_pSettings->tanScreenUnit2Unit(vsgPIXELUNIT, x, vsgDEGREEUNIT, &xVoltage);
				g_pSettings->tanScreenUnit2Unit(vsgPIXELUNIT, y-dAmplitudePix, vsgDEGREEUNIT, &yVoltage);
				xVoltage=xVoltage/g_pSettings->getDegPerVolt();
				yVoltage=yVoltage/g_pSettings->getDegPerVolt();
			}
			double xbase,ybase;
		...
		/* Optokinetic after nystagmus timing
		//blank period

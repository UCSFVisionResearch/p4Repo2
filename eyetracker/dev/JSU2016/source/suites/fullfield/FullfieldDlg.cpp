...
void CFullfieldDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_RadioCir, m_Cir);
	DDX_Control(pDX, IDC_EditFreq, m_CirFreq);
	DDX_Control(pDX, IDC_EditAmp, m_CirAmp);
}
...
BEGIN_MESSAGE_MAP(CFullfieldDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_RadioCir, OnBnClickedRadioCir)
END_MESSAGE_MAP()
...
void CFullfieldDlg::load() 
{
	...
	switch (g_pFullfield->getDriftDirection()) {
		...
		case CFullfield::DIR_CIR: OnBnClickedRadioCir(); break;
	}
	...
	m_CirFreq.SetWindowTextA(etu::FloatToString(g_pFullfield->getCirFreq()));
	m_CirAmp.SetWindowTextA(etu::FloatToString(g_pFullfield->getCirAmp()));
}
...
void CFullfieldDlg::save() 
{
	...
	else if (m_Cir.GetCheck() == BST_CHECKED) {
		g_pFullfield->setDriftDirection(CFullfield::DIR_CIR);
	}
	m_CirFreq.GetWindowText(buffer, 100);
	g_pFullfield->setCirFreq(etu::StringToFloat(buffer, g_pFullfield->getCirFreq()));

	m_CirAmp.GetWindowText(buffer, 100);
	g_pFullfield->setCirAmp(etu::StringToFloat(buffer, g_pFullfield->getCirAmp()));
...
void CFullfieldDlg::OnBnClickedRadioup()
{
	...
	m_Cir.SetCheck(BST_UNCHECKED);
	m_CirFreq.EnableWindow(false);
	m_CirAmp.EnableWindow(false);
}
...
void CFullfieldDlg::OnBnClickedRadioCir()
{
	m_Up.SetCheck(BST_UNCHECKED);
	m_Down.SetCheck(BST_UNCHECKED);
	m_Left.SetCheck(BST_UNCHECKED);
	m_Right.SetCheck(BST_UNCHECKED);
	m_Cir.SetCheck(BST_CHECKED);
	m_CirFreq.EnableWindow(true);
	m_CirAmp.EnableWindow(true);
}

...
public:
	...
	CRetinotopicTrialGenerator(const CTrialSaccades& trialSaccades,
		etu::pGrid_t& fixationGrid, etu::pGrid_t& peripheralGrid, 
		etu::pGrid_t& peripheralGridL, pEyePositions_t& eyePositions);
...
CSaccadeTrialGenerator* CSaccadeTrialGenerator::GetGenerator(
	...
	etu::pGrid_t& peripheralGrid, etu::pGrid_t& peripheralGridL)
{
	if (trialSaccades.getEyeCoverage() == CTrialSaccades::COVERAGE_RETINOTOPIC)
	{
		...
		return (new CRetinotopicTrialGenerator(trialSaccades, fixationGrid,
			peripheralGrid, peripheralGridL, eyePositions));
	}
	else {
...
CRetinotopicTrialGenerator::CRetinotopicTrialGenerator(
	const CTrialSaccades& trialSaccades, etu::pGrid_t& fixationGrid, 
	etu::pGrid_t& peripheralGrid,etu::pGrid_t& peripheralGridL, pEyePositions_t& eyePositions)
: m_trialSaccades(trialSaccades), m_fixationGrid(fixationGrid), 
  m_peripheralGrid(peripheralGrid), m_peripheralGridL(peripheralGridL), m_eyePositions(eyePositions)
{}
...
void CRetinotopicTrialGenerator::GenerateTrials(TrialPtrContainer_t& trials)
{
	...
	const etu::GridIter_t perEndL = m_peripheralGridL->end();
	...
	bool stimRight = g_pTrialSaccades->getStimRightEye();
	bool stimLeft = g_pTrialSaccades->getStimLeftEye();
	
	etu::GridIter_t fixIter = m_fixationGrid->begin();
	etu::GridIter_t perIter=m_peripheralGrid->begin();
	etu::GridIter_t perIterL=m_peripheralGridL->begin();

	for ( ; fixIter != fixEnd; ++fixIter, perIter = m_peripheralGrid->begin(),perIterL=m_peripheralGridL->begin()) 
	{
		...
		{	
			...
			{
				int numRepeats, ix;
				if(stimRight||(!stimRight&& !stimLeft)){
					pSaccadeTrial = newTrial(CSaccadeTrial::EYE_RIGHT,
				...
				if(!stimRight&& !stimLeft){
					pSaccadeTrial = newTrial(CSaccadeTrial::EYE_LEFT,
						(CSaccadeTrial::STIM_CODE)code, m_eyePositions.get(), *perIter);
		...
		if(stimLeft)
		{
			...
			{	
				for (int code = CSaccadeTrial::CODE_C_RED_P_RED; code <= CSaccadeTrial::CODE_C_BLUE_P_BLUE; ++code)
				{
					...
					pSaccadeTrial = newTrial(CSaccadeTrial::EYE_LEFT,
						(CSaccadeTrial::STIM_CODE)code, m_eyePositions.get(), *perIterL);

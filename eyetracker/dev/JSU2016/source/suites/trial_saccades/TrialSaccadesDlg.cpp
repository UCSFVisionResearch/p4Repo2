...
char* DIFFICULTY_DESCRIPTIONS[CTrialSaccades::NUM_DIFFICULTY_LEVELS] = {
	"No cross-overs shown" ,
	"25% of normal cross-overs shown"   ,
	"Half the normal cross-overs shown" ,
	"75% of normal cross-overs shown"   ,
	"All cross-overs shown" };
...
void CTrialSaccadesDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_tbsStimRightEye, m_StimRightEye);
	DDX_Control(pDX, IDC_tbsStimLeftEye, m_StimLeftEye);
	DDX_Control(pDX, IDC_tbsShowRightEye, m_ShowRightEye);
	DDX_Control(pDX, IDC_tbsShowLeftEye, m_ShowLeftEye);
	DDX_Control(pDX, IDC_tbsSaveGrid, m_SaveGrid);
	DDX_Control(pDX, IDC_tbsAngleOffset, m_AngleOffset);
	DDX_Control(pDX, IDC_tbsAngleFlux, m_AngleFlux);
	...
	DDX_Control(pDX, IDC_tbsRadiusFlux, m_RadiusFlux);
...
BEGIN_MESSAGE_MAP(CTrialSaccadesDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_tbsStimRightEye, OnBnClickedStimRightEye)
	ON_BN_CLICKED(IDC_tbsStimLeftEye, OnBnClickedStimLeftEye)
	ON_BN_CLICKED(IDC_tbsShowRightEye, OnBnClickedShowRightEye)
	ON_BN_CLICKED(IDC_tbsShowLeftEye, OnBnClickedShowLeftEye)
	ON_BN_CLICKED(IDC_tbsSaveGrid, OnBnClickedSaveGrid)
...
void CTrialSaccadesDlg::load() 
{
	...
	m_StimRightEye.SetCheck((g_pTrialSaccades->getStimRightEye()) ? BST_CHECKED : BST_UNCHECKED);
	m_StimLeftEye.SetCheck((g_pTrialSaccades->getStimLeftEye()) ? BST_CHECKED : BST_UNCHECKED);
	OnBnClickedStimLeftEye();
	OnBnClickedStimRightEye();
...
void CTrialSaccadesDlg::save() 
{
	...
	if (m_GridRadioCartesian.GetCheck() == BST_CHECKED) {
		...
		g_pTrialSaccades->setStimRightEye(m_StimRightEye.GetCheck() ==BST_CHECKED);
		g_pTrialSaccades->setStimLeftEye(m_StimLeftEye.GetCheck()==BST_CHECKED);
		OnBnClickedSaveGrid(); //Saves whichever Eye is shown
	}
...
//Test if viewing maximum times is more intuitive
	fixationTime = ((m_RandomizeDisplayTime.GetCheck() == BST_CHECKED) ? 
		GetAverageTime(m_DisplayRandMax, m_DisplayRandMax) : 
		GetAverageTime(m_DisplayTime, m_DisplayTime));
	cueTime = GetAverageTime(m_CueTimeMax, m_CueTimeMax);
	delayTime = GetAverageTime(m_DelayTimeMax, m_DelayTimeMax);
	lingerTime = GetAverageTime(m_LingerTimeMax, m_LingerTimeMax);
...
void CTrialSaccadesDlg::EnableGridRadialControls(bool enable)
{
	...
	m_StimRightEye.EnableWindow(enable);
	m_AngleOffset.EnableWindow(enable);
	m_AngleFlux.EnableWindow(enable);
	...
	m_RadiusFlux.EnableWindow(enable);
...
void CTrialSaccadesDlg::OnBnClickedStimLeftEye(){
	if(m_StimLeftEye.GetCheck()){
		m_ShowLeftEye.EnableWindow(true);
		OnBnClickedShowLeftEye();
	}else if(m_StimRightEye.GetCheck()){
		m_ShowLeftEye.EnableWindow(false);
		OnBnClickedShowRightEye();
	}else{
		m_ShowLeftEye.EnableWindow(false);
		OnBnClickedShowRightEye();
		m_SaveGrid.SetWindowTextA("Save Both");
	}
}

void CTrialSaccadesDlg::OnBnClickedStimRightEye(){
	if(m_StimRightEye.GetCheck()){
		m_ShowRightEye.EnableWindow(true);
		OnBnClickedShowRightEye();
	}else if(m_StimLeftEye.GetCheck()){
		m_ShowRightEye.EnableWindow(false);
		OnBnClickedShowLeftEye();
	}else{
		m_ShowRightEye.EnableWindow(false);
		OnBnClickedShowRightEye();
...
void CTrialSaccadesDlg::OnBnClickedShowLeftEye(){
	m_SaveGrid.SetWindowTextA("Save Left");
	bShowLeft = true;
	m_AngleOffset.SetWindowText(etu::IntToString(g_pTrialSaccades->getAngleOffsetL()));
	m_AngleFlux.SetWindowTextA(etu::IntToString(g_pTrialSaccades->getAngleFluxL()));
	m_Angle0.SetCheck((g_pTrialSaccades->getAngle0L()) ? BST_CHECKED : BST_UNCHECKED);
	m_Angle45.SetCheck((g_pTrialSaccades->getAngle45L()) ? BST_CHECKED : BST_UNCHECKED);
	m_Angle90.SetCheck((g_pTrialSaccades->getAngle90L()) ? BST_CHECKED : BST_UNCHECKED);
	m_Angle135.SetCheck((g_pTrialSaccades->getAngle135L()) ? BST_CHECKED : BST_UNCHECKED);
	m_Angle180.SetCheck((g_pTrialSaccades->getAngle180L()) ? BST_CHECKED : BST_UNCHECKED);
	m_Angle225.SetCheck((g_pTrialSaccades->getAngle225L()) ? BST_CHECKED : BST_UNCHECKED);
	m_Angle270.SetCheck((g_pTrialSaccades->getAngle270L()) ? BST_CHECKED : BST_UNCHECKED);
	m_Angle315.SetCheck((g_pTrialSaccades->getAngle315L()) ? BST_CHECKED : BST_UNCHECKED);
	m_RadiusOffset.SetWindowText(etu::IntToString(g_pTrialSaccades->getRadiusOffsetL()));
	m_RadiusFlux.SetWindowText(etu::IntToString(g_pTrialSaccades->getRadiusFluxL()));
	m_RadiusOrigin.SetCheck((g_pTrialSaccades->getRadiusOriginL()) ? BST_CHECKED : BST_UNCHECKED);
	m_Radius0.SetCheck((g_pTrialSaccades->getRadius0L()) ? BST_CHECKED : BST_UNCHECKED);
	m_Radius5.SetCheck((g_pTrialSaccades->getRadius5L()) ? BST_CHECKED : BST_UNCHECKED);
	m_Radius10.SetCheck((g_pTrialSaccades->getRadius10L()) ? BST_CHECKED : BST_UNCHECKED);
	m_Radius15.SetCheck((g_pTrialSaccades->getRadius15L()) ? BST_CHECKED : BST_UNCHECKED);
	m_Radius20.SetCheck((g_pTrialSaccades->getRadius20L()) ? BST_CHECKED : BST_UNCHECKED);
	m_Radius25.SetCheck((g_pTrialSaccades->getRadius25L()) ? BST_CHECKED : BST_UNCHECKED);
	m_Radius30.SetCheck((g_pTrialSaccades->getRadius30L()) ? BST_CHECKED : BST_UNCHECKED);
}

void CTrialSaccadesDlg::OnBnClickedShowRightEye(){
	m_SaveGrid.SetWindowTextA("Save Right");
	bShowLeft = false;
	m_AngleOffset.SetWindowText(etu::IntToString(g_pTrialSaccades->getAngleOffsetL()));
	m_AngleFlux.SetWindowTextA(etu::IntToString(g_pTrialSaccades->getAngleFluxL()));
	...
	m_RadiusFlux.SetWindowText(etu::IntToString(g_pTrialSaccades->getRadiusFlux()));
...
void CTrialSaccadesDlg::OnBnClickedSaveGrid(){
	char buffer[100];
	if (bShowLeft){
		m_AngleOffset.GetWindowText(buffer, 100);
		g_pTrialSaccades->setAngleOffsetL(etu::StringToInt(buffer, g_pTrialSaccades->getAngleOffsetL()));
		m_AngleOffset.SetWindowText(etu::IntToString(g_pTrialSaccades->getAngleOffsetL()));
		m_AngleFlux.GetWindowText(buffer, 100);
		g_pTrialSaccades->setAngleFluxL(etu::StringToInt(buffer, g_pTrialSaccades->getAngleFluxL()));
		m_AngleFlux.SetWindowText(etu::IntToString(g_pTrialSaccades->getAngleFluxL()));
		g_pTrialSaccades->setAngle0L(m_Angle0.GetCheck() == BST_CHECKED);
		g_pTrialSaccades->setAngle45L(m_Angle45.GetCheck() == BST_CHECKED);
		g_pTrialSaccades->setAngle90L(m_Angle90.GetCheck() == BST_CHECKED);
		g_pTrialSaccades->setAngle135L(m_Angle135.GetCheck() == BST_CHECKED);
		g_pTrialSaccades->setAngle180L(m_Angle180.GetCheck() == BST_CHECKED);
		g_pTrialSaccades->setAngle225L(m_Angle225.GetCheck() == BST_CHECKED);
		g_pTrialSaccades->setAngle270L(m_Angle270.GetCheck() == BST_CHECKED);
		g_pTrialSaccades->setAngle315L(m_Angle315.GetCheck() == BST_CHECKED);

		m_RadiusOffset.GetWindowText(buffer, 100);
		g_pTrialSaccades->setRadiusOffsetL(etu::StringToInt(buffer, g_pTrialSaccades->getRadiusOffsetL()));
		m_RadiusOffset.SetWindowText(etu::IntToString(g_pTrialSaccades->getRadiusOffsetL()));
		m_RadiusFlux.GetWindowText(buffer, 100);
		g_pTrialSaccades->setRadiusFluxL(etu::StringToInt(buffer, g_pTrialSaccades->getRadiusFluxL()));
		m_RadiusFlux.SetWindowText(etu::IntToString(g_pTrialSaccades->getRadiusFluxL()));
		g_pTrialSaccades->setRadiusOriginL(m_RadiusOrigin.GetCheck() == BST_CHECKED);
		g_pTrialSaccades->setRadius0L(m_Radius0.GetCheck() == BST_CHECKED);
		g_pTrialSaccades->setRadius5L(m_Radius5.GetCheck() == BST_CHECKED);
		g_pTrialSaccades->setRadius10L(m_Radius10.GetCheck() == BST_CHECKED);
		g_pTrialSaccades->setRadius15L(m_Radius15.GetCheck() == BST_CHECKED);
		g_pTrialSaccades->setRadius20L(m_Radius20.GetCheck() == BST_CHECKED);
		g_pTrialSaccades->setRadius25L(m_Radius25.GetCheck() == BST_CHECKED);
		g_pTrialSaccades->setRadius30L(m_Radius30.GetCheck() == BST_CHECKED);
	}else{
		m_AngleOffset.GetWindowText(buffer, 100);
		...
		m_AngleFlux.GetWindowText(buffer, 100);
		g_pTrialSaccades->setAngleFlux(etu::StringToInt(buffer, g_pTrialSaccades->getAngleFlux()));
		m_AngleFlux.SetWindowText(etu::IntToString(g_pTrialSaccades->getAngleFlux()));
		...
		m_RadiusOffset.SetWindowText(etu::IntToString(g_pTrialSaccades->getRadiusOffset()));
		m_RadiusFlux.GetWindowText(buffer, 100);
		g_pTrialSaccades->setRadiusFlux(etu::StringToInt(buffer, g_pTrialSaccades->getRadiusFlux()));
		m_RadiusFlux.SetWindowText(etu::IntToString(g_pTrialSaccades->getRadiusFlux()));

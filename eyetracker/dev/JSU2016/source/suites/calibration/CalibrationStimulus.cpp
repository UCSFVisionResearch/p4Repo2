...
CCalibrationStimulus::CCalibrationStimulus() 
{
	...
	etu::AnalogOut(4, 0.0); //Sets target color to 0. Recording target color not important in this suite.
...
bool CCalibrationStimulus::DisplayStimulus() 
{
	...
	double dSize;
	double hSeparationPix, vSeparationPix;
	...
	g_pSettings->tanScreenUnit2Unit(vsgDEGREEUNIT, g_pCalibration->getHSeparation(), vsgPIXELUNIT, &hSeparationPix);
	g_pSettings->tanScreenUnit2Unit(vsgDEGREEUNIT, g_pCalibration->getVSeparation(), vsgPIXELUNIT, &vSeparationPix);
	...
	if (g_pCalibration->showAll()) {
		...
		switch (iCalibrationType) {
			case 2:
				for (int i = 0; i < iCalibrationType; i++) {
					vsgDrawOval(stimXOffset + screenXOffset+(TWO_PT_CAL[i].x * hSeparationPix), stimYOffset + screenYOffset + (TWO_PT_CAL[i].y * vSeparationPix), dSize, dSize);
				}		
				break;
			case 5:
				for (int i = 0; i < iCalibrationType; i++) {
					vsgDrawOval(stimXOffset + screenXOffset+(FIVE_PT_CAL[i].x * hSeparationPix), stimYOffset + screenYOffset + (FIVE_PT_CAL[i].y * vSeparationPix), dSize, dSize);
				}
				break;
			case 9:
				for (int i = 0; i < iCalibrationType; i++) {
					vsgDrawOval(stimXOffset + screenXOffset+(NINE_PT_CAL[i].x * hSeparationPix), stimYOffset + screenYOffset + (NINE_PT_CAL[i].y * vSeparationPix), dSize, dSize);
				}
				break;
		}
		xDeg = 0;
		...
		xPix *= (int)hSeparationPix;
		yPix *= (int)vSeparationPix;
...
void CCalibrationStimulus::DoCycle() 
{
	...
	for (int i = 0; i < 9; i++) {
		vsgSetDrawPage(vsgVIDEOPAGE, iPage, vsgBACKGROUND);
		vsgSetDrawMode(vsgCENTREXY);
		vsgDrawOval(screenXOffset+(NINE_PT_CAL[i].x * hSeparationPix), screenYOffset + (NINE_PT_CAL[i].y * vSeparationPix), dSize, dSize);
		double xPix = NINE_PT_CAL[i].x * hSeparationPix;
		double yPix = NINE_PT_CAL[i].y * vSeparationPix;

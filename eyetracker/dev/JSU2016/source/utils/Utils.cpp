 ...
 * If buffer is not a valid number, oldValue is returned.
 * @param buffer String representing a number.
 * @param oldValue Value to return if buffer is not a valid number.
 * @return Buffer string as an int, or oldValue.
 */
int StringToInt(char* buffer, int oldValue) 
{
	char* endPtr;
	int ii = Round(strtod(buffer, &endPtr));
	if (strlen(endPtr) > 0) {

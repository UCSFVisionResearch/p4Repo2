        ...
        if (bRet == -1) {
            ...
            DispatchMessage(&msg); 
				...
				if (msg.message == WM_KEYDOWN) {
					...
					else if (g_pEyeTrackingDlg->m_pTabCtrl.GetCurSel()==9) {//RF Mapping Dialog
						if (msg.wParam == VK_RIGHT) {
							/* VK_RIGHT
							 * Activate Right Shutter */
							g_pEyeTrackingDlg->OnBnClickedShutterr();
						} 
						else if (msg.wParam == VK_LEFT) {
							/* VK_LEFT
							 * Activate Left Shutter*/
							g_pEyeTrackingDlg->OnBnClickedShutterl();
						}
					...
					else if (g_pEyeTrackingDlg->m_pTabCtrl.GetCurSel()==1) {// Calibration Dialog
						if (msg.wParam == VK_RIGHT) {
							/* VK_RIGHT
							 * Activate Right Shutter */
							g_pEyeTrackingDlg->OnBnClickedShutterr();

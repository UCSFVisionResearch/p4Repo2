...
void CEyeTrackingDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_ReverseShutters, m_ReverseShutters);
}
...
BEGIN_MESSAGE_MAP(CEyeTrackingDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_ReverseShutters, OnBnClickedReverseShutters)
END_MESSAGE_MAP()
...
void CEyeTrackingDlg::AddTabs() 
{
	...
	m_ReverseShutters.SetWindowPos(0, buttonLeft+1190, buttonTop+75, buttonWidth, buttonHeight, SWP_NOSIZE);
	...
	char* tabTwentyFour = "Reverse Fixation";
	char* tabTwentyFive = "SparseNoise";
	...
	tcItem.pszText = tabTwentyFour;
	m_pTabCtrl.InsertItem(24, &tcItem);

	tcItem.pszText = tabTwentyFive;
	m_pTabCtrl.InsertItem(25, &tcItem);
	...
	g_pReverseFixationDlg = new CReverseFixationDlg();
	g_pReverseFixationDlg->Create(IDD_REVERSEFIX_DLG, this);

	g_pSparseNoiseDlg = new CSparseNoiseDlg();
	g_pSparseNoiseDlg->Create(IDD_SPARSE_NOISE_DLG, this);
#endif
...
void CEyeTrackingDlg::OnTcnSelchangeTabs(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	switch (iCurrSel) {
		...
		case 24: g_pReverseFixationDlg->ShowWindow(SW_SHOW); m_Run.EnableWindow(true); break;
		case 25: g_pSparseNoiseDlg->ShowWindow(SW_SHOW); m_Run.EnableWindow(true); break;
	#endif
	#ifdef APP_ACUITY
...
void CEyeTrackingDlg::OnTcnSelchangingTabs(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	switch (iCurrSel) {
		...
		case 24: g_pReverseFixationDlg->ShowWindow(SW_HIDE); break;
		case 25: g_pSparseNoiseDlg->ShowWindow(SW_HIDE); break;
	#endif
	#ifdef APP_ACUITY
...
void CEyeTrackingDlg::OnBnClickedRun()
{
	...
	if (!g_pActiveStimulus) {
		...
		switch (iCurrSel) {
			...
			case 24: pActiveThread = AfxBeginThread(ReverseFixationControllingFunction, &m_Run, THREAD_PRIORITY_NORMAL, 0, 0, NULL); break;
			case 25: pActiveThread = AfxBeginThread(SparseNoiseControllingFunction, &m_Run, THREAD_PRIORITY_NORMAL, 0, 0, NULL); break;
		#endif
		#ifdef APP_ACUITY
...
UINT __cdecl ReverseFixationControllingFunction(LPVOID pParam) 
{
	...
	// Create the saccade stimulus object
	g_pActiveStimulus = new CReverseFixationStimulus();
...
UINT __cdecl SparseNoiseControllingFunction(LPVOID pParam) 
{
	...
	// Create the saccade stimulus object
	g_pActiveStimulus = new CSparseNoiseStimulus();
...
void CEyeTrackingDlg::OnBnClickedLoadsettings()
{
	...
	switch (iCurrSel) {
		...
		case 24:g_pReverseFixation->LoadFile(false); g_pReverseFixationDlg->load(); break;
		case 25:g_pSparseNoise->LoadFile(false); g_pSparseNoiseDlg->load(); break;
#endif
#ifdef APP_ACUITY
...
void CEyeTrackingDlg::OnBnClickedSavesettings()
{
	...
	switch (iCurrSel) {
		...
		case 24:g_pReverseFixation->SaveFile(false); break;
		case 25:g_pSparseNoise->SaveFile(false); break;
#endif
#ifdef APP_ACUITY
...
void CEyeTrackingDlg::OnBnClickedMakedefault()
{
	...
	switch (iCurrSel) {
		...
		case 24:g_pReverseFixation->SaveFile(true); break;
		case 25:g_pSparseNoise->SaveFile(true); break;
#endif
#ifdef APP_ACUITY
...
void CEyeTrackingDlg::OnBnClickedLoaddefault()
{
	...
	switch (iCurrSel) {
		...
		case 24:g_pReverseFixation->LoadFile(true); g_pReverseFixationDlg->load(); break;
		case 25:g_pSparseNoise->LoadFile(true); g_pSparseNoiseDlg->load(); break;
#endif
#ifdef APP_ACUITY
...
void CEyeTrackingDlg::OnBnClickedReverseShutters()
{
	OnBnClickedShutterl();
	OnBnClickedShutterr();
}

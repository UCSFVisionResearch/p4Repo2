...
{
	save();
	g_receptiveFieldLeft.xPos = g_pRFMapping->getXPosition() - g_pSettings->getScreenXOffset();
	g_receptiveFieldLeft.yPos = g_pRFMapping->getYPosition() - g_pSettings->getScreenYOffset();
...
{
	save();
	g_receptiveFieldRight.xPos = g_pRFMapping->getXPosition() - g_pSettings->getScreenXOffset();
	g_receptiveFieldRight.yPos = g_pRFMapping->getYPosition() - g_pSettings->getScreenYOffset();
...
void CRFMappingDlg::OnMouseMove(UINT nFlags, CPoint point)
{
	if (g_pRFMapping->useMouse() && g_pActiveStimulus  && g_pActiveStimulus->StimulusType() == "RF Mapping Stimulus") {
		if (MK_LBUTTON & nFlags) {
			...
			/** 
			 * @TODO Remove silly code.  
			 *
			 * Because lastPoint is later set equal to point, this vector math
			 * reduces to 
			 *     currPoint = point
			 * This does give a correct call to updatePosition.
			 */
			currPoint.x = min(pixW, max(currPoint.x + point.x - lastPoint.x, 0));
			currPoint.y = min(pixH, max(currPoint.y + point.y - lastPoint.y, 0));

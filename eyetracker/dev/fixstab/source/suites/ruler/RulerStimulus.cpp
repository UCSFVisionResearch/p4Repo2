...
bool CRulerStimulus::DisplayStimulus() 
{
	...
	if (g_pRuler->getBox()==BST_CHECKED) {
		double a,b;
		g_pSettings->tanScreenUnit2Unit(vsgDEGREEUNIT, 30.0, vsgPIXELUNIT, &a);
		g_pSettings->tanScreenUnit2Unit(vsgDEGREEUNIT, 60.0, vsgPIXELUNIT, &b);
		vsgSetPen1(0);//dot color
		vsgDrawRect(stimXOffset+screenXOffset,screenYOffset+stimYOffset,b,a);

		g_pSettings->tanScreenUnit2Unit(vsgDEGREEUNIT, 29.0, vsgPIXELUNIT, &a);
		g_pSettings->tanScreenUnit2Unit(vsgDEGREEUNIT, 59.0, vsgPIXELUNIT, &b);

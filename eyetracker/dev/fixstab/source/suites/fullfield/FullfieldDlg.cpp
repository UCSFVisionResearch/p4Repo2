...
void CFullfieldDlg::load() 
{
	...
	m_Contrast.SetWindowText(etu::IntToString(g_pFullfield->getContrast()));
	...
	if (g_pFullfield->useFixation()) {
		m_CheckFixation.SetCheck(BST_CHECKED);
	} 
	else {
		m_CheckFixation.SetCheck(BST_UNCHECKED);
	}
	if (g_pFullfield->getNoise()) {
		m_Noise.SetCheck(BST_CHECKED);
	} 
	else {
		m_Noise.SetCheck(BST_UNCHECKED);
	}
...
void CFullfieldDlg::save() 
{
	...
	g_pFullfield->setContrast(etu::StringToInt(buffer, g_pFullfield->getContrast()));

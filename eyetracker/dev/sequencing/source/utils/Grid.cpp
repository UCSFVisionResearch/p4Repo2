 ...
 * @param xOffsetDeg If non-zero perform vector addition of this offset to all
 *        points in the grid.
 * @param yOffsetDeg If non-zero, perform vector addition of this offset to 
 *        all points in the grid.
 */
pGrid_t createCartesianGrid(int stepsX, int stepsY, double spaceDeg, 
							bool zeroPt, double xOffsetDeg, 
							double yOffsetDeg)
{
	...
	for (int xx = -stepsX; xx <= stepsX; xx++) {
		for (int yy = -stepsY; yy <= stepsY; yy++) {
			...
			if (zeroPt || xx || yy) {
				xDeg = xx * spaceDeg + xOffsetDeg;
				yDeg = yy * spaceDeg + yOffsetDeg; 

...
//MKF 4/15/09 added for HZ update
// takes a time string and duration (integer in minutes), and formats it according to type
// returns the formatted string
function formatted_time_range($time, $dur, $type)
{
	global $config;
	...
	switch($type) {
		default:
			...
			return sprintf('%d:%02d%s-%d:%02d%s', $hour1, $minute1, $pm1, $hour2, $minute2, $pm2);
		case 2:
			return _('FULL DAY');
		case 3:
			return _('TBA');
...
function navbar()
{
	...
	if(is_user() && $action != 'search') {//MKF 4/15/09 added is_user()

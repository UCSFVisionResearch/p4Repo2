...
function display()
{
	...
	if(!is_user()) return "";//MKF 4/15/09 must be logged in to see anything
...
function month_navbar($month, $year)
{
	...
	//MKF 4/15/09 removed for HZ, Proctor Confocal Calendar
	//for($i = 1; $i <= 12; $i++) {
...
function display_days($day_count, $week_of_month, $month, $year)
{
	...
	if($day_of_month <= 0 || $day_of_month > days_in_month($month, $year)) {
		...
		while($row = $result->FetchRow($result)) {
			...
			//		$row['starttime'],
			//		$row['eventtype']);
			//MKF update 4/15/09, this one shows on month-view calendar
			$event_time = formatted_time_range($row['starttime'], $row['duration'], $row['eventtype']);

$row2 = get_event_by_id($row['id']);//MKF 4/17/2009 use this function to get the username field
			...
			$event = tag('li',
                                        tag('a',
                                                ...
                                                ($event_time ? "$event_time - "
                                                 : '')
                                                . $row2['username']));//MKF 4/17/2009 replace $subject with username
...
//MKF 4/17/2009 add username to subject
$row2 = get_event_by_id($row['uid']);
$subject = $subject ." - ". $row2['username'];
			$desc = parse_desc($row['description']);
			//$time_str = formatted_time_string($row['starttime'],
			//		$row['eventtype']);
//MKF 4/15/09 change for HZ add end time to display
			$time_str = formatted_time_range($row['starttime'], $row['duration'], $row['eventtype']);
